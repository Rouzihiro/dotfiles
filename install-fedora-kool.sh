...
LOG="Install-Logs/01-Fedora-Install-Scripts-$(date +%d-%H%M%S).log"
...
whiptail --title "Rey Fedora-Sway (2025) Install Script" \
    --msgbox "Welcome to Rey Fedora-Sway (2025) Install Script!!!\n\n\
ATTENTION: Run a full system update and Reboot first !!! (Highly Recommended)\n\n\
NOTE: If you are installing on a VM, ensure to enable 3D acceleration else Sway may NOT start!" \
    15 80
...
echo "👌 ${OK} 🇵🇭 ${MAGENTA}Rey...${RESET} ${SKY_BLUE}lets coninue with the installation...${RESET}" | tee -a "$LOG"
...
options_command+=(
    ...
    "xdph" "Install XDG-DESKTOP-PORTAL-WLR (for screen share)?" "OFF"
    "zsh" "Install zsh shell with Oh-My-Zsh?" "OFF"
    "pokemon" "Add Pokemon color scripts to your terminal?" "OFF"
    "rog" "Are you installing on Asus ROG laptops?" "OFF"
    "dots" "Download and install pre-configured Rey Sway dotfiles?" "OFF"
)
...
while true; do
    ...
    if [[ "$dots_selected" == "OFF" ]]; then
        ...
        if ! whiptail --title "Rey Sway Dot Files" --yesno \
        "You have not selected to install the pre-configured Rey Sway dotfiles.\n\nKindly NOTE that if you proceed without Dots, Sway will start with default vanilla Sway configuration and I won't be able to give you support.\n\nWould you like to continue install without Rey Sway Dots or return to choices/options?" \
    ...
    echo "👌 ${OK} You confirmed your choices. Proceeding with ${SKY_BLUE}Rey 🇵🇭 Sway Installation...${RESET}" | tee -a "$LOG"
    break  
done
...
for option in "${options[@]}"; do
    case "$option" in
        ...
        quickshell)
            ...
            ;;
        xdph)
            echo "${INFO} Installing ${SKY_BLUE}xdg-desktop-portal-Sway...${RESET}" | tee -a "$LOG"
...
# Check if Sway or Sway-git is installed
if rpm -q Sway &> /dev/null || rpm -q Sway-git &> /dev/null; then
    printf "\n ${OK} 👌 Sway is installed. However, some essential packages may not be installed. Please see above!"
    printf "\n${CAT} Ignore this message if it states ${YELLOW}All essential packages${RESET} are installed as per above\n"
    sleep 2
    printf "\n%.0s" {1..2}

    printf "${SKY_BLUE}Thank you${RESET} 🫰 for using 🇵🇭 ${MAGENTA}Rey's Sway Dots${RESET}. ${YELLOW}Enjoy and Have a good day!${RESET}"
    printf "\n%.0s" {1..2}

    printf "\n${NOTE} You can start Sway by typing ${SKY_BLUE}Sway${RESET} (IF SDDM is not installed) (note the capital H!).\n"
    ...
    printf "\n${WARN} Sway is NOT installed. Please check 00_CHECK-time_installed.log and other files in the Install-Logs/ directory..."
    printf "\n%.0s" {1..3}
    exit 1
fi
