...
in {
  programs.tmux = {
    ...
    extraConfig = ''
      		...
      		set -ga terminal-overrides ",foot*:Tc,foot:RGB"
      		set -ga terminal-overrides ",xterm-256color:RGB"
      		set -g escape-time 0  # Faster response in Wayland
      ...
      # Left status (session name) - Fixed connection
      set -g status-left "#[fg=#3B4252,bg=default]#[fg=#D8DEE9,bg=#3B4252] #S #[fg=#3B4252,bg=#434C5E]"

      # Right status - With thin Starship-style transitions
      set -g status-right "\
