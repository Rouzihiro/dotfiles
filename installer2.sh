...
PACKAGE_DIR="install"  # Directory containing package lists locally
...
# Check for required command, install if missing
check_command() {
    local cmd="$1"
    local pkg="$2"
...
# Fetch package lists dynamically from GitHub and read packages
fetch_and_read_packages() {
  local repo_path="$1"      # e.g. install/pacman or install/aur
  local raw_base_url="https://raw.githubusercontent.com/Rouzihiro/dotfiles/main"
  local -n array_ref="$2"
  ...
  local api_url="https://api.github.com/repos/Rouzihiro/dotfiles/contents/$repo_path"

  local file_list
  file_list=$(curl -s "$api_url" | jq -r '.[] | select(.type=="file") | .name')

  if [[ -z "$file_list" ]]; then
    log_warning "No package files found in $repo_path on GitHub"
    return 1
  fi
  ...
  for file in $file_list; do
    local url="$raw_base_url/$repo_path/$file"
    local tmpfile="$temp_dir/$file"
    ...
    if curl -fsSL "$url" -o "$tmpfile"; then
      ...
      log_info "Loaded remote package list: $file"
    else
      log_warning "Failed to download $url"
    fi
  done

  # Cleanup temp_dir on exit
...
main() {
    ...
    # Use local package directories if present, else fallback to remote
    if [[ -d "$PACMAN_DIR" ]]; then
        select_packages "$PACMAN_DIR" "Pacman" PACMAN_PACKAGES
    else
        log_warning "Pacman package directory not found locally, falling back to remote lists"
        fetch_and_read_packages "install/pacman" PACMAN_PACKAGES
    fi
    
    if [[ -d "$AUR_DIR" ]]; then
        select_packages "$AUR_DIR" "AUR" AUR_PACKAGES
    else
        log_warning "AUR package directory not found locally, falling back to remote lists"
        fetch_and_read_packages "install/aur" AUR_PACKAGES
    fi
