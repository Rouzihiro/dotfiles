...
DMENU_CMD="dmenu"
DMENU_OPTS="-i -l 20 -c"  # centered, 20 lines
...
while true; do
    ...
    while IFS= read -r file; do
        ...
        [ ${#short} -gt $MAXLEN ] && short="${short:0:$((MAXLEN-1))}…"
        display+="$short"$'\n'
        map["$short"]="$file"
    done <<< "$pdfs"

    # Show menu with title line
    choice=$(printf "Books\n---\n%s\n" "$display" | ${DMENU_CMD} ${DMENU_OPTS} 2>/dev/null)

    # Exit if user picked Quit, ESC, or closed dmenu
