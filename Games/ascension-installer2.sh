...
function install_winetricks_deps() {
  ...
  winetricks -q win10 ie8 corefonts dotnet48 vcrun2015 || {
    echo "[!] Winetricks failed. Try running manually for debugging."
  }
}
...
function download_launcher() {
  ...
  mkdir -p "$INSTALL_DIR"
  cd "$INSTALL_DIR"
  wget -q "$INSTALLER_URL" -O "$INSTALLER_NAME"
}
...
function print_run_instruction() {
  ...
  echo "Please finish installation via GUI, then run the game using:"
  echo ""
...
echo "== Unattended Ascension WoW Setup =="
