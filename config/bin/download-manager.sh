...
parse_progress() {
    local raw="$1"
    local tail_chunk
    tail_chunk=$(tail -c 4000 "$raw" 2>/dev/null | tr '\r' '\n')

    # Find whichever came last: a real progress line, or aria2c's completion summary
    local last_relevant
    last_relevant=$(echo "$tail_chunk" | grep -E '%|Download Results:' | tail -1)

    if [ -z "$last_relevant" ]; then
        echo "0|-|-"
        return
    fi

    if [[ "$last_relevant" == *"Download Results:"* ]]; then
        echo "100|-|-"
        return
    fi

    local last="$last_relevant"
    local pct speed eta
...
render_dashboard() {
    ...
    if [ "${#metas[@]}" -eq 0 ]; then
        frame=$'💤 Idle — watching clipboard for links...\n'
        printed=1
    else
        for meta in "${metas[@]}"; do
            ...
            if [ -f "$done_file" ]; then
                ...
                frame+="$(printf "✅ %-40s complete" "${name:0:40}")"$'\n'
                ticks=$((ticks - 1))
                if [ "$ticks" -le 0 ]; then
                ...
                frame+="$(printf "❌ %-40s failed" "${name:0:40}")"$'\n'
                ticks=$((ticks - 1))
                if [ "$ticks" -le 0 ]; then
                ...
                bar=$(make_bar "$pct")
                frame+="$(printf "⬇️  %-30s %s %3s%%  %10s  ETA %s" "${name:0:30}" "$bar" "$pct" "$speed" "$eta")"$'\n'
            fi
            printed=$((printed + 1))
        done
    fi
...
local move_up=""
    if [ "$LAST_LINES" -gt 0 ]; then
        move_up="$(tput cuu "$LAST_LINES")$(tput ed)"
    fi
    printf "%s%s" "$move_up" "$frame"
...
printf "Keys: q = quit (downloads keep running)   Q = quit and cancel all downloads\n\n"
fi
...
while true; do
    ...
    case "$key" in
        q) break ;;
        Q) kill $(jobs -rp) 2>/dev/null; break ;;
    esac
done
