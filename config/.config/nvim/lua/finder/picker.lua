...
function M.open(source, prompt, callback)
  ...
  -- The search field must start genuinely empty.
  -- Do not add cosmetic spaces here because they become real characters.
  ...
  local function update_preview()
    ...
    -- Only preview real regular files.
    ...
    if not ok or not lines then
      set_preview_message("Unable to preview file.")
      return
    end

    -- nvim_buf_set_lines() requires one physical line per table item.
    -- Normalize anything containing embedded newlines.
    lines = util.normalize_lines(lines)
    ...
    -- Filetype detection is also protected because some unusual files
    -- can make filetype detection/read operations fail.
    local filetype_ok, filetype = pcall(
      util.get_filetype,
      path
    )

    if filetype_ok and filetype then
  ...
  local function open_with(cmd)
    -- If still focused on search, move focus to results.
    if searching then
      focus_results()
    end
    ...
    -- Pass the open mode to callback:
    -- "split", "vsplit", "diff", or nil.
    callback(path, cmd)
  end
