-- Custom Neovim colorscheme — Crimson
-- Black peonies, blood-red roses, gold filaments in the dark.
...
-- ─── Palette ──────────────────────────────────────────────────────────────────
local c = {
  bg          = "#0a0605",  -- near-black, the void behind the petals
  bg_subtle   = "#1c0e10",  -- barely-lit petal shadow
  bg_muted    = "#3a2426",  -- dark oxblood, cracked petal edge
  bg_sel      = "#2a1214",  -- deep wine for selections
  fg          = "#d9c9a3",  -- pale gold — stamen filaments
  fg_dim      = "#8f7d61",  -- dulled gold, receding
  fg_faint    = "#504036",  -- ash taupe, near-invisible
  silver      = "#c9a961",  -- warm brass — bright filament highlight
  grey_olive  = "#8f6d3c",  -- bronze-olive, muted gold
  slate       = "#a3583f",  -- rust — burnt petal undertone
  copper      = "#b8860f",  -- warm amber-gold
  iron        = "#8a1b2c",  -- crimson rose — the accent blood-red
  taupe       = "#3a2426",  -- same as bg_muted, explicit alias
  none        = "NONE",
}
...
hi("String",           { fg = c.fg })         -- pale gold — parchment text
...
hi("@string",                  { fg = c.fg })       -- pale gold, the main warmth
...
-- ─── Diff ─────────────────────────────────────────────────────────────────────
hi("DiffAdd",    { fg = c.slate,  bg = "#150e0a" })
hi("DiffChange", { fg = c.copper, bg = "#1e1508" })
hi("DiffDelete", { fg = c.iron,   bg = "#1a0709" })
hi("DiffText",   { fg = c.fg,     bg = "#2e1c08",  bold = true })
hi("Added",      { fg = c.slate })
